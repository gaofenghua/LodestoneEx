    ...
    public class Job : Base.ServiceJob
    {
        ...
        private void ACAPService_DataUpdatedEvent(object sender, EventArgs e)
        {
            ...
            if ((null != arg) && (null != arg.Info))
            {
                ...
                cam.SetCameraStatus(DEVICE_STATE.DEVICE_ONLINE);

                Jobs.AVMS.Job avms = m_parentJob as Jobs.AVMS.Job;
                if ((null != avms) && (avms.IsRunning) && (null != avms.m_deviceFilter))
                {
                    bool isChanged = false;
                    avms.m_deviceFilter.UpdateACAPCameras(cam, ref isChanged);
                }
            }
        }
        ...
        protected override void Start()
        {
            ...
            try
            {
                ...
                m_serviceHost = new WebServiceHost(m_service, baseAddress);   //or WebServiceHost _serviceHost = new WebServiceHost(typeof(PersonInfoQueryServices), baseAddress);
                ...
                {
                    PrintLog("GAT1400 service has been opened...");
                };
                //
                m_serviceHost.Open();
